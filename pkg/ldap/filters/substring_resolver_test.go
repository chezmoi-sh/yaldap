	...
	tests := []filterResolverTestCase{
		{
			...
			filter: must(goldap.CompileFilter("(memberOf=ad*)")),
			result: Some(true),
		},
		{
			name:   "SubstringsAnySucceed",
			filter: must(goldap.CompileFilter("(memberOf=*dm*)")),
			result: Some(true),
		},
		{
			name:   "SubstringsFinalSucceed",
			filter: must(goldap.CompileFilter("(memberOf=*min)")),
			result: Some(true),
		},
		{
			name:   "SubstringsFullSucceed",
			filter: must(goldap.CompileFilter("(memberOf=a*m*n)")),
			result: Some(true),
		},
		{
			name:   "SubstringsInitialFailed",
			filter: must(goldap.CompileFilter("(memberOf=un*)")),
			result: Some(false),
		},
		{
			name:   "SubstringsAnyFailed",
			filter: must(goldap.CompileFilter("(memberOf=*kno*)")),
			result: Some(false),
		},
		{
			name:   "SubstringsFinalFailed",
			filter: must(goldap.CompileFilter("(memberOf=*own)")),
			result: Some(false),
		},
		{
			name:   "SubstringsFullFailed",
			filter: must(goldap.CompileFilter("(memberOf=u*k*n)")),
			result: Some(false),
		},
